// prisma/seeds/add-orders-export-permission.ts
//
// Mục đích: CHỈ thêm (upsert) đúng MỘT quyền `orders:export` vào bảng
...
//
// Cách chạy:  yarn seed:orders-export
//
...
const PERMISSION = {
  ...
  action: 'export',
  scope: 'all',
  description: 'Xuất file đơn đặt hàng',
  // Phải trùng category của các quyền orders:* gốc để nằm CHUNG box trên màn
  // phân quyền (grouping = category + resource).
  category: 'Bán hàng',
};

async function main() {
  console.log('🌱 Thêm quyền orders:export (chỉ 1 quyền, không ảnh hưởng dữ liệu khác)...');
...
main()
  ...
  .catch((e) => {
    console.error('❌ Lỗi khi thêm quyền orders:export:', e);
    process.exit(1);
  })
