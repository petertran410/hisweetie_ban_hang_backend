// prisma/seeds/add-purchase-orders-export-permission.ts
//
// Mục đích: CHỈ thêm (upsert) đúng MỘT quyền `purchase_orders:export` vào bảng
...
//
// Cách chạy:  yarn seed:purchase-orders-export
//
...
const PERMISSION = {
  name: 'purchase_orders:export',
  resource: 'purchase_orders',
  action: 'export',
  scope: 'all',
  description: 'Xuất file nhập hàng',
  // Phải trùng category của các quyền purchase_orders:* gốc để nằm CHUNG box
...
async function main() {
  console.log(
    '🌱 Thêm quyền purchase_orders:export (chỉ 1 quyền, không ảnh hưởng dữ liệu khác)...',
  );
...
main()
  ...
  .catch((e) => {
    console.error('❌ Lỗi khi thêm quyền purchase_orders:export:', e);
    process.exit(1);
  })
