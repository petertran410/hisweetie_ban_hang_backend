// prisma/seeds/add-inventory-checks-export-permission.ts
//
// Mục đích: CHỈ thêm (upsert) đúng MỘT quyền `inventory_checks:export` vào bảng
...
//
// Cách chạy:  yarn seed:inventory-checks-export
//
...
async function main() {
  console.log(
    '🌱 Thêm quyền inventory_checks:export (chỉ 1 quyền, không ảnh hưởng dữ liệu khác)...',
  );
...
main()
  ...
  .catch((e) => {
    console.error('❌ Lỗi khi thêm quyền inventory_checks:export:', e);
    process.exit(1);
  })
