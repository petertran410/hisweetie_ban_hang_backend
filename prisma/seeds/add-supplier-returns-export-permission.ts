// prisma/seeds/add-supplier-returns-export-permission.ts
//
// Mục đích: CHỈ thêm (upsert) đúng MỘT quyền `supplier_returns:export` vào bảng
...
//
// Cách chạy:  yarn seed:supplier-returns-export
//
...
const PERMISSION = {
  name: 'supplier_returns:export',
  resource: 'supplier_returns',
  action: 'export',
  scope: 'all',
  description: 'Xuất file trả hàng nhập',
  // Phải trùng category của các quyền supplier_returns:* gốc để nằm CHUNG box
...
async function main() {
  console.log(
    '🌱 Thêm quyền supplier_returns:export (chỉ 1 quyền, không ảnh hưởng dữ liệu khác)...',
  );
...
main()
  ...
  .catch((e) => {
    console.error('❌ Lỗi khi thêm quyền supplier_returns:export:', e);
    process.exit(1);
  })
