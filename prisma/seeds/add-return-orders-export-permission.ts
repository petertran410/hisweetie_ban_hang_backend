// prisma/seeds/add-return-orders-export-permission.ts
//
// Mục đích: CHỈ thêm (upsert) đúng MỘT quyền `return_orders:export` vào bảng
...
//
// Cách chạy:  yarn seed:return-orders-export
//
...
const PERMISSION = {
  name: 'return_orders:export',
  resource: 'return_orders',
  action: 'export',
  scope: 'all',
  description: 'Xuất file trả hàng',
  // Phải trùng category của các quyền return_orders:* gốc để nằm CHUNG box
...
async function main() {
  console.log(
    '🌱 Thêm quyền return_orders:export (chỉ 1 quyền, không ảnh hưởng dữ liệu khác)...',
  );
...
main()
  ...
  .catch((e) => {
    console.error('❌ Lỗi khi thêm quyền return_orders:export:', e);
    process.exit(1);
  })
