// prisma/seeds/add-vat-invoices-export-permission.ts
//
// Mục đích: CHỈ thêm (upsert) đúng MỘT quyền `vat_invoices:export` vào bảng
...
//
// Cách chạy:  yarn seed:vat-invoices-export
//
...
const PERMISSION = {
  name: 'vat_invoices:export',
  resource: 'vat_invoices',
  action: 'export',
  scope: 'all',
  description: 'Xuất file hóa đơn VAT',
  category: 'Hóa đơn VAT',
};

async function main() {
  console.log(
    '🌱 Thêm quyền vat_invoices:export (chỉ 1 quyền, không ảnh hưởng dữ liệu khác)...',
  );
...
main()
  ...
  .catch((e) => {
    console.error('❌ Lỗi khi thêm quyền vat_invoices:export:', e);
    process.exit(1);
  })
