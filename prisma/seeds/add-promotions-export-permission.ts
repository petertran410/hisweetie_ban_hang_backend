// prisma/seeds/add-promotions-export-permission.ts
//
// Mục đích: CHỈ thêm (upsert) đúng MỘT quyền `promotions:export` vào bảng
...
//
// Cách chạy:  yarn seed:promotions-export
//
...
const PERMISSION = {
  ...
  action: 'export',
  scope: 'all',
  description: 'Xuất file khuyến mãi',
  // Phải trùng category của các quyền promotions:* gốc để nằm CHUNG box trên
  // màn phân quyền (grouping = category + resource).
  category: 'Khách hàng',
};

async function main() {
  console.log(
    '🌱 Thêm quyền promotions:export (chỉ 1 quyền, không ảnh hưởng dữ liệu khác)...',
  );
...
main()
  ...
  .catch((e) => {
    console.error('❌ Lỗi khi thêm quyền promotions:export:', e);
    process.exit(1);
  })
