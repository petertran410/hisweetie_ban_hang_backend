...
import { config, type McpClientConfig } from './config.js';
...
declare global {
  namespace Express {
    interface Request { mcpAuth?: AuthContext }
  }
}

function findClient(clientId: string, clientSecret: string): McpClientConfig | undefined {
  return config.MCP_CLIENTS.find((client) => client.clientId === clientId && client.clientSecret === clientSecret);
}

export async function issueAccessToken(client: McpClientConfig): Promise<string> {
...
export function requireScope(req: Request, scope: string): void {
  if (!req.mcpAuth?.scopes.includes(scope)) throw new Error(`Missing OAuth scope: ${scope}`);
}
