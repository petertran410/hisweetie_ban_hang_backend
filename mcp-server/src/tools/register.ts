...
function register(
  ...
  server.registerTool(name, {
    ...
    if (!write) return result(await handler(input));
    const idempotencyKey = String(input.idempotencyKey ?? '');
    const cacheKey = `mcp:idempotency:${name}:${idempotencyKey}`;
    const cached = await redis.get(cacheKey);
    if (cached) return result(JSON.parse(cached));
    const lockKey = `${cacheKey}:lock`;
    const acquired = await redis.set(lockKey, '1', 'EX', 60, 'NX');
    if (!acquired) throw new Error('This idempotent operation is already in progress');
    try {
...
export function registerTools(server: McpServer): void {
  ...
  }, (input) => posRequest('GET', '/customers', { query: input, branchId: input.branchId as number }));
  ...
  }, (input) => posRequest('GET', '/customers/totals', { query: input, branchId: input.branchId as number }));
  ...
  }, (input) => posRequest('GET', '/reports/customer/debt-documents', { query: input, branchId: input.branchId as number }));
  ...
  }, (input) => posRequest('GET', '/products', { query: input, branchId: input.branchId as number }));

  register(server, 'crm_get_product', 'Get product', 'Get product details by numeric ID.', {
    id: z.number().int().positive(), branchId,
  }, ({ id, branchId }) => posRequest('GET', `/products/${id}`, { branchId: branchId as number }));
  ...
  register(server, 'crm_get_branch_inventory', 'Get branch inventory', 'Get inventory rows for one branch and optional product IDs.', {
    ...
    query: { branchId, productIds: (productIds as number[] | undefined)?.join(',') }, branchId: branchId as number,
  }));
  ...
  }, ({ productId, ...query }) => posRequest('GET', `/products/${productId}/inventory-logs`, { query, branchId: query.branchId as number }));
  ...
  }, (input) => posRequest('GET', '/orders', { query: input, branchId: input.branchId as number }));

  register(server, 'crm_get_order', 'Get order', 'Get complete order details.', {
    id: z.number().int().positive(), branchId,
  }, ({ id, branchId }) => posRequest('GET', `/orders/${id}`, { branchId: branchId as number }));
  ...
  }, ({ order }) => posRequest('POST', '/orders', { body: order, branchId: (order as Record<string, unknown>).branchId as number }), true);
  ...
  }, ({ id, order }) => posRequest('PUT', `/orders/${id}`, { body: order, branchId: (order as Record<string, unknown>).branchId as number }), true);
  ...
  }, (input) => posRequest('GET', '/orders/product-price-history', { query: input, branchId: input.branchId as number }));
  ...
  }, (input) => posRequest('GET', '/invoices', { query: input, branchId: input.branchId as number }));

  register(server, 'crm_get_invoice', 'Get invoice', 'Get complete invoice details.', {
    id: z.number().int().positive(), branchId,
  }, ({ id, branchId }) => posRequest('GET', `/invoices/${id}`, { branchId: branchId as number }));
  ...
  }, (input) => posRequest('GET', '/reports/customer/preview', { query: input, branchId: input.branchId as number }));

  register(server, 'crm_list_accessible_branches', 'List accessible branches', 'List branches available to the configured POS service account.', {},
    () => posRequest('GET', '/branches/my-branches'));
}
