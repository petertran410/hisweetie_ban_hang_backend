...
/**
 * Service riêng cho base Sản Phẩm (LARK_PRODUCT_BASE_TOKEN — base có sẵn
 * sản phẩm + bảng giá + tồn kho). Tách khỏi LarkBaseService (khoá vào base
 * Order) để tránh lẫn token. Cấu trúc method mirror LarkCustomerBaseService.
 */
@Injectable()
export class LarkProductBaseService {
  private readonly logger = new Logger(LarkProductBaseService.name);
  ...
  constructor(
    ...
    this.baseToken = this.config.get<string>('LARK_PRODUCT_BASE_TOKEN') || null;
    if (!this.baseToken) {
      this.logger.warn(
        'LARK_PRODUCT_BASE_TOKEN chưa cấu hình — sync sản phẩm sẽ bị skip',
      );
    }
  }
  ...
  /**
   * Lấy tập tên field hợp lệ của table (có phân trang).
   * Dùng để whitelist payload → tránh lỗi FieldNameNotFound làm fail cả batch.
   * Trả về Set rỗng nếu lỗi (caller nên fallback: gửi nguyên payload).
   */
  async fetchFieldNames(tableId: string): Promise<Set<string>> {
    const names = new Set<string>();
    if (!this.baseToken) return names;
    ...
    while (hasMore) {
      try {
        ...
        this.logger.error(`Fetch field names failed: ${error.message}`);
        break;
      }
    }
  ...
  async createRecord(
    ...
    try {
      ...
      this.logger.error(`Create product record failed: ${error.message}`);
      throw error;
    }
  }
  ...
  async updateRecord(
    ...
    try {
      ...
      this.logger.error(
        `Update product record ${recordId} failed: ${error.message}`,
      );
      throw error;
    }
  }
