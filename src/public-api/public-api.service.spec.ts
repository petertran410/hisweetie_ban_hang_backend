...
describe('PublicApiService', () => {
  const createService = () => {
    ...
    const prisma: any = {
      ...
      customerGroup: delegate(),
      customer: delegate(),
      product: delegate(),
      inventory: delegate(),
      order: delegate(),
      invoice: delegate(),
      returnOrder: delegate(),
      category: delegate(),
      tradeMark: delegate(),
      saleChannel: delegate(),
      bankAccount: delegate(),
      user: delegate(),
      supplier: delegate(),
      supplierGroup: delegate(),
      priceBook: delegate(),
      ...
      location: delegate(),
      settings: delegate(),
      orderSupplier: delegate(),
      consignment: delegate(),
    ...
    prisma.$transaction = jest.fn((operations: Promise<unknown>[]) =>
      Promise.all(operations),
    );
    return { prisma, service: new PublicApiService(prisma) };
  };
  ...
  it('trả envelope KiotViet với total/pageSize/currentItem/data/timestamp', async () => {
    ...
    const result = await service.list('branches', {
      pageSize: 2,
      currentItem: 4,
    } as any);
  ...
  it('lọc delta sync theo lastModifiedFrom trên cột updatedAt', async () => {
    ...
    await service.list('customers', {
      lastModifiedFrom: timestamp.toISOString(),
    } as any);
  ...
  it('trả lỗi not found nhất quán khi chi tiết resource không tồn tại', async () => {
    ...
    await expect(service.get('invoices', 99)).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
  ...
  it('dùng đúng cột isActivate cho kênh bán hàng', async () => {
    ...
    expect(prisma.saleChannel.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
