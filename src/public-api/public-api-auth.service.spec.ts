...
describe('PublicApiAuthService', () => {
  ...
  afterAll(() => {
    if (originalPublicSecret === undefined)
      delete process.env.PUBLIC_API_JWT_SECRET;
  ...
  it('rejects missing client credentials before accessing storage', async () => {
    ...
    await expect(
      service.issueToken({ grantType: 'client_credentials' }),
  ...
  it.each([
    ...
    if (bcryptResult !== undefined)
      compare.mockResolvedValue(bcryptResult as never);
  ...
  it('issues a typed token with client credentials and requested scope', async () => {
    ...
    expect(prisma.publicApiClient.findUnique).toHaveBeenCalledWith({
  ...
  it.each([
    ...
    'clamps a client token TTL of %i seconds to %i seconds',
    async (storedTtl, expectedTtl) => {
      const client = {
        id: 'client-id',
        clientId: 'partner',
        clientSecret: 'hash',
