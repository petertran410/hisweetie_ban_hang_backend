...
describe('CustomerDemandImportService', () => {
  ...
  it('preview giữ các dòng trùng SKU trong cùng một phiếu', async () => {
    const file = await excelFile([
    ...
    expect(preview.rows[2].voucherIndex).toBe(1);
    expect(preview.groups[0].lines.map((line) => line.productId)).toEqual([
      11, 12, 11,
    ]);
  });

  it('preview nhận tháng 2026-9 và giữ hai dòng trùng SKU', async () => {
    const file = await excelFile([
    ...
    expect(preview.rows[0].voucherIndex).toBe(1);
    expect(preview.rows[1].voucherIndex).toBe(1);
  });
