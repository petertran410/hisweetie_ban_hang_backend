...
export class PaymentNotesService {
  ...
  async createOrderNote(orderId: number, dto: CreatePaymentNoteDto, userId: number) {
    this.validate(dto);
    ...
    return this.prisma.orderPaymentNote.create({
      data: { orderId, paymentType: dto.paymentType, amount: dto.paymentType === 'cash' ? dto.amount : null, createdBy: userId },
    });
  }

  async createInvoiceNote(invoiceId: number, dto: CreatePaymentNoteDto, userId: number) {
    this.validate(dto);
    ...
    return this.prisma.invoicePaymentNote.create({
      data: { invoiceId, paymentType: dto.paymentType, amount: dto.paymentType === 'cash' ? dto.amount : null, createdBy: userId },
    });
  }
}
