...
export class SyncKiotController {
   ...
   * Không update SyncControl.
   */
  @Post('invoices/by-date-range')
  async syncInvoicesByDateRange(
    @Body() body?: { fromDate?: string; toDate?: string },
  ) {
    if (!(await this.syncService.isSyncEnabled())) {
    ...
    this.logger.log(
      `📨 Manual sync invoices where purchaseDate in [${fromDate.toISOString()}, ${toDate.toISOString()}]`,
    );
    const results = await this.syncService.runInvoicesByDateRange(
      fromDate,
      toDate,
    );
    return {
